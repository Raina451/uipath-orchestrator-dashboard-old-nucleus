...
- **Dashboard Overview** - Real-time metrics and activity feed for quick insights
- **Process Management** - View and start automation processes with folder scoping
- **Job Monitoring** - Comprehensive job tracking with status indicators and logs
...
VITE_UIPATH_SCOPE=OR.Execution OR.Jobs OR.Folders OR.Assets OR.Robots
```
...
2. Click the login button to initiate OAuth flow
...
- **Jobs** - Monitor job executions and view logs
...
Most resources in Orchestrator are folder-scoped. Use the folder selector dropdown in the navigation bar to switch between different folders and view their respective resources.
